...
Get warnings when translations are missing for specific languages. Configurable severity and language filtering.
...
Multiple `useTranslation()` calls with different namespaces are fully supported.

### Monorepo Support

![Monorepo support](docs/images/monorepo.png)

Automatic per-package server isolation based on `package.json` boundaries.
